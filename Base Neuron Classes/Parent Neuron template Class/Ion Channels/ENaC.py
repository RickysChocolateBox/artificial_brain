...
class ENaC(IonChannel):
    ...
    def steady_state_values(self, voltage):
        m_inf = 1 / (1 + np.exp(-(voltage + 45) / 15))
        h_inf = 1 / (1 + np.exp((voltage + 60) / 5))
        return m_inf, h_inf

    def time_constants(self, voltage):
        tau_m = 1 / (np.exp((voltage + 60) / 15) + np.exp(-(voltage + 60) / 15))
        tau_h = 1 / (np.exp((voltage + 80) / 5) + np.exp(-(voltage + 80) / 5))
        return tau_m, tau_h
