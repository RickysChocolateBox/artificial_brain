...
class InhibitoryInterneuronWorkingMemory:
    def __init__(self, num_neurons, inhibition_strength=0.5, working_threshold=0.6, learning_rate=0.01, plasticity_rate=0.1):
        ...
        self.working_weights = np.random.uniform(0, 1, num_neurons)
        self.working_trace = np.zeros(num_neurons)
    ...
    def update_working_weights(self, activity_levels):
        self.working_trace = self.plasticity_rate * activity_levels + (1 - self.plasticity_rate) * self.working_trace
        self.working_weights += self.learning_rate * (activity_levels - self.working_weights) * self.working_trace

    def update_activity(self, activity_levels):
        high_activity_neurons = activity_levels >= self.working_threshold
        ...
        self.update_working_weights(activity_levels)
        return activity_levels

    def working_neurons(self, activity_levels):
        return np.argwhere(activity_levels >= self.working_threshold).flatten()
