...
class InhibitoryInterneuronCooperation:
    def __init__(self, num_neurons, initial_connectivity=None, cooperation_rate=0.1, min_strength=-1, max_strength=0):
    ...
    def update_cooperation(self, activity_levels, min_strength, max_strength):
        cooperation = np.outer(activity_levels, activity_levels)
        self.connectivity -= self.cooperation_rate * cooperation
        self.connectivity = np.clip(self.connectivity, min_strength, max_strength)

    def apply_cooperation(self, activity_levels):
        self.update_cooperation(activity_levels, self.min_strength, self.max_strength)
