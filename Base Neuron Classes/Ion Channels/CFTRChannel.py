...
class CFTRChannel(IonChannel):
    ...
    def steady_state_values(self, voltage):
        ...
        h_inf = 1 / (1 + np.exp(-(voltage + 50) / 5))
        return m_inf, h_inf

    def time_constants(self, voltage):
        tau_m = 1 + 4 / (np.exp((voltage + 20) / 20) + np.exp(-(voltage + 20) / 20))
        tau_h = 10 + 5 / (np.exp((voltage + 30) / 10) + np.exp(-(voltage + 30) / 10))
        return tau_m, tau_h
