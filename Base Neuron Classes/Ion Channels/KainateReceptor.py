...
class IonChannel:
    ...
    def compute_current(self, voltage):
        pass
class KainateReceptor(IonChannel):
    def __init__(self, conductance):
        self.conductance = conductance
        self.activation = 0.0
    ...
    def steady_state_values(self, voltage):
        m_inf = 1 / (1 + np.exp(-(voltage + 15) / 10))
        h_inf = 1 / (1 + np.exp((voltage + 25) / 5))
        return m_inf, h_inf

    def time_constants(self, voltage):
        tau_m = 1 / (np.exp((voltage + 25) / 10) + np.exp(-(voltage + 25) / 10))
        tau_h = 1 / (np.exp((voltage + 35) / 5) + np.exp(-(voltage + 35) / 5))
        return tau_m, tau_h
    ...
    def compute_current(self, voltage):
        return self.conductance * self.activation * self.inactivation * (voltage - 15)
