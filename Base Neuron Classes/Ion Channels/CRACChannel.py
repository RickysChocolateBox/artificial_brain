...
class CRACChannel(IonChannel):
    ...
    def steady_state_values(self, voltage):
        m_inf = 1 / (1 + np.exp((voltage + 50) / 7))
        h_inf = 1 / (1 + np.exp(-(voltage + 60) / 4))
        return m_inf, h_inf

    def time_constants(self, voltage):
        tau_m = 1 + 1 / (np.exp((voltage + 30) / 15) + np.exp(-(voltage + 30) / 15))
        tau_h = 10 + 3 / (np.exp((voltage + 40) / 8) + np.exp(-(voltage + 40) / 8))
        return tau_m, tau_h
