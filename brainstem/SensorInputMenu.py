...
from HumiditySensorSource import HumiditySensorSource
    ...
    def run(self):
        while True:
            ...
            if main_choice == 1:
                ...
                print("11. HumiditySensorSource")
                choice = int(input("Enter the number corresponding to your choice: "))

                # Rest of the code for adding sensors remains the same...
                ...
                if choice == 1:
                    ...
                    self.brainstem.add_sensory_source("stereo_camera", stereo_vision_source)
                elif choice == 2:
                     ...
                     Humidity_sensor_source = HumiditySensorSource()
                     Brainstem.add_sensory_source("Humidity", Humidity_sensor_source)


                elif main_choice == 2:
                     motor_name = input("Enter a name for the motor (e.g., left_arm, right_leg): ")
                     motor = Motor(motor_name)
                self.brainstem.add_motor(motor_name, motor)

                # Automatically add the corresponding sensor
                motor_joint_position_sensor_source = MotorJointPositionSensorSource(motor)
                self.brainstem.add_sensory_source(f"{motor_name}_joint_position", motor_joint_position_sensor_source)
...
if __name__ == "__main__":
    sensor_input_menu = SensorInputMenu()
    sensor_input_menu.run()