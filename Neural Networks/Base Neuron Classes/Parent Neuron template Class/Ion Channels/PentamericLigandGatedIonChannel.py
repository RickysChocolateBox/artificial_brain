...
class IonChannel:
    ...
    def compute_current(self, voltage):
        pass
class PentamericLigandGatedIonChannel(IonChannel):
    def __init__(self, conductance):
    ...
    def steady_state_values(self, voltage):
        ...
        h_inf = 1 / (1 + np.exp((voltage + 40) / 5))
        return m_inf, h_inf

    def time_constants(self, voltage):
        tau_m = 10 + 5 / (np.exp((voltage + 20) / 10) + np.exp(-(voltage + 20) / 10))
        tau_h = 30 + 10 / (np.exp((voltage + 30) / 5) + np.exp(-(voltage + 30) / 5))
        return tau_m, tau_h
    ...
    def compute_current(self, voltage):
        return self.conductance * self.activation * self.inactivation * (voltage - 10)