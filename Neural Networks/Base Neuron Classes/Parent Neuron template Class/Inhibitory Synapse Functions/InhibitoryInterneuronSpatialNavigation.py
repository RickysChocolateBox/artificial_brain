...
class InhibitoryInterneuronSpatialNavigation:
    ...
    def update_activity(self, current_activity, spatial_input, dt):
        spatial_related_neurons = np.abs(current_activity - spatial_input) <= self.inhibition_strength
        inhibition = self.compute_inhibition(current_activity, spatial_related_neurons)
        new_activity = current_activity - inhibition * dt
        new_activity.clip(min=0, max=1, out=new_activity)
        return new_activity

    def integrate_spatial_input(self, activity_levels, spatial_input, dt):
        next_activity_levels = self.update_activity(activity_levels, spatial_input, dt)
