...
class InhibitoryInterneuronMotorControl:
    def __init__(self, num_neurons, inhibition_strength=0.5, learning_rate=0.01, motor_threshold=0.6):
        ...
        self.motor_weights = np.random.uniform(0, 1, num_neurons)
    ...
    def update_activity(self, activity_levels):
        high_activity_neurons = activity_levels >= self.motor_threshold
    ...
    def motor_neurons(self, activity_levels):
        return np.argwhere(activity_levels >= self.motor_threshold).flatten()
